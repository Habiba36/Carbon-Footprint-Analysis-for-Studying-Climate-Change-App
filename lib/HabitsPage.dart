...
class _seletscreenState extends State<seletscreen> {
  ...
  seq()async{
    ...
    for(int i =0; i < widget.habit.length; i++){
      DateTime sDate = startDateMap[widget.habit[i].index.toString()];
      //print(sDate.toString());
      DateTime endDate = sDate.add(const Duration(days:21));
      //print(endDate.toString());
      //print(widget.habit[i].index.toString());
      print("==================================");
      await getDaysInBetween(sDate, endDate,widget.habit[i].index.toString());
      await get(widget.habit[i].index.toString());
            ...
            children: <Widget>[
              ...
              Expanded(
                child: ListView.separated(
                  itemCount: widget.habit.length,
                  separatorBuilder: (BuildContext context, int index) {
                    return SizedBox(height: 20.0);
                  },
                  itemBuilder: (ctx, index) {
                    habits h = widget.habit[index];
